...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!payload.job_posting_id) {
      ...
      return NextResponse.json(
        { error: 'Missing job_posting_id' },
        { status: 400 }
      )
    }

    // Check if job_posting_id is a valid UUID format
    const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i
    if (!uuidRegex.test(payload.job_posting_id)) {
      console.error('❌ Invalid UUID format for job_posting_id:', payload.job_posting_id)
      
      // If it's a test ID, try to find the most recent job instead
      if (payload.job_posting_id.includes('test')) {
        console.log('🔍 Test job ID detected, finding most recent job...')
        ...
        if (jobsError || !recentJobs || recentJobs.length === 0) {
          console.error('❌ No recent jobs found:', jobsError)
          return NextResponse.json(
            { error: 'No recent jobs found to associate with test data' },
            { status: 400 }
          )
        }
        
        const actualJobId = recentJobs[0].id
        console.log('✅ Found recent job ID:', actualJobId)
        payload.job_posting_id = actualJobId
      } else {
        return NextResponse.json(
          { error: 'Invalid job_posting_id format - must be a valid UUID' },
          { status: 400 }
        )
      }
    }
    ...
    return NextResponse.json({ 
      ...
      applicantsProcessed: payload.applicants?.length || 0
    })
